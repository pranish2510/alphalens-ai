...
export default function StockPanel({ symbol, onStockLoad, onAlertsLoad }) {
  ...
  async function fetchAll(sym) {
    ...
    try {
      ...
      // ── Stage 1b: Chart data (parallel, non-blocking) ────────────────────
      fetch(`/api/chart?symbol=${encodeURIComponent(sym)}&range=3mo`, { signal })
        .then(r => r.json())
        .then(d => { if (!signal.aborted && d.success) setChart(d); })
        .catch(() => {});
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '32px 28px 40px', display: 'flex', flexDirection: 'column', gap: '36px' }}>
        ...
        {stock && !loading && (
          <>
            ...
            <PriceChart symbol={symbol} chart={chart} stock={stock} />
            {thesis && (
              <ThesisBlock
...
/* ─── Price Chart ─────────────────────────────────────────────────────────── */
function PriceChart({ symbol, chart, stock }) {
  const [range, setRange]     = useState('3mo');
  const [data, setData]       = useState(chart);
  const [loading, setLoading] = useState(false);
  const W = 680, H = 200, PAD = { top: 14, right: 14, bottom: 28, left: 52 };

  useEffect(() => { if (chart) setData(chart); }, [chart]);

  const fetchRange = async (r) => {
    setRange(r);
    setLoading(true);
    try {
      const res = await fetch(`/api/chart?symbol=${encodeURIComponent(symbol)}&range=${r}`);
  ...
  const renderChart = () => {
    ...
    const gradId = `cg${symbol.replace(/\W/g, '')}`;

    const pts = closes.map((c, i) => ({
      x: PAD.left + (i / (closes.length - 1)) * cW,
      y: PAD.top  + (1 - (c - minC) / span) * cH,
    }));
    ...
    const step = Math.max(1, Math.floor((candles.length - 1) / 3));
    const xIdxs = [0, step, step * 2, candles.length - 1].filter((v, i, a) => a.indexOf(v) === i);
    ...
    return (
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block', overflow: 'visible' }}>
        ...
        {yTicks.map((t, i) => (
          <g key={i}>
            ...
            <text x={PAD.left - 6} y={t.y + 3.5} textAnchor="end"
              fontSize="9" fontFamily="monospace" fill="var(--muted)">
              {fmt(t.val)}
            </text>
          </g>
        ))}
        ...
        {/* X labels */}
        {xIdxs.map((idx, i) => (
          <text key={i} x={pts[idx].x} y={H - 5} textAnchor="middle"
            fontSize="9" fontFamily="monospace" fill="var(--muted)">
        ...
        <path d={linePath} fill="none" stroke={lineColor} strokeWidth="1.6" strokeLinejoin="round" />

        {/* Current price dot */}
        <circle cx={pts[pts.length - 1].x} cy={pts[pts.length - 1].y} r="3.5"
          fill={lineColor} stroke="var(--bg-raised)" strokeWidth="2" />
      </svg>
    );
  };
    ...
    <section>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
        ...
        <div style={{ display: 'flex', gap: '4px' }}>
          {RANGES.map(r => (
            <button key={r} onClick={() => fetchRange(r)} style={{
              fontFamily: 'var(--mono)', fontSize: '9px', padding: '3px 9px', borderRadius: '2px',
              cursor: 'pointer', letterSpacing: '0.06em', transition: 'all 0.12s',
              background: range === r ? 'var(--beige)' : 'var(--bg-surface)',
              color:      range === r ? 'var(--bg)'    : 'var(--muted)',
              border: `1px solid ${range === r ? 'var(--beige)' : 'var(--border)'}`,
      ...
      <div style={{ background: 'var(--bg-raised)', border: '1px solid var(--border)', borderRadius: 'var(--r-md)', padding: '16px', position: 'relative', minHeight: 220 }}>
        {loading && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 'var(--r-md)', zIndex: 2, background: 'rgba(0,0,0,0.25)' }}>
            <span style={{ fontFamily: 'var(--mono)', fontSize: '10px', color: 'var(--muted)' }}>Loading…</span>
          </div>
        )}
        {candles.length < 2 && !loading ? (
          <div className="skeleton" style={{ height: 188, borderRadius: 'var(--r-sm)', opacity: 0.3 }} />
        ) : renderChart()}
      </div>
    </section>
  );
